...
let currentDetailsData = {}; // NEW: To store data for "Load More" functionality

// --- NEW: Global variables for graph state ---
...
function renderPersonDetails(person) {
    ...
    // Only show details if a death date is present
...
async function buildQueryAndFetch() {
    // --- NEW: Divert to graph logic if 'graphs' is selected ---
    ...
    document.querySelectorAll('.filter-row.sub-row').forEach(row => {
        ...
        if (field && logic && multiselect && !dateFilter && !authorshipSearchFilter && !manifestationRoleFilter && !textSearchFilter) {
        ...
        // --- MODIFICATION START: Handle person dates with era ---
        else if (field === 'person_dates' && dateFilter) {
            const fromVal = dateFilter.querySelector('.date-from').value;
            const toVal = dateFilter.querySelector('.date-to').value;
            if (fromVal) {
                const fromEra = dateFilter.querySelector('.era-from').value;
                query.rules.push({ field: 'person_birth_date', logic: 'gte', value: fromVal, era: fromEra });
            }
            if (toVal) {
                const toEra = dateFilter.querySelector('.era-to').value;
                query.rules.push({ field: 'person_death_date', logic: 'lte', value: toVal, era: toEra });
            }
        }
        // --- MODIFICATION END ---
        ...
        else if (field === 'visual_object_transcription' && logic && textSearchFilter) {
...
function displayResults(results, entity) {
    ...
    if (entity === 'work') {
        ...
        resultsContent.innerHTML = results.map(vo => {
            const card = vo.card;
            const transcriptionSnippet = card.transcription_snippet ?
                `<p><strong>Transcription:</strong> <em>${card.transcription_snippet}</em></p>` : '';
            return `
            <div class="result-card">
                <div class="card-header">
    ...
    container.innerHTML = `
        ...
        <div class="custom-select-wrapper">
            <select class="text-search-operator">
                ...
                <option value="exact_phrase">Complete phrase</option>
            </select>
        </div>
    `;
        ...
        if (field) {
            ...
            if (field === 'publication_date') {
                valuePlaceholder.innerHTML = `
                    <div class="date-filter-container">
                        <label>From:</label>
                        <input type="number" class="date-from" placeholder="Year">
                        <label>To:</label>
                        <input type="number" class="date-to" placeholder="Year">
                    </div>
                `;
                ...
                // --- MODIFICATION START: Changed input type to text and updated placeholder ---
                valuePlaceholder.innerHTML = `
                    <div class="date-filter-container">
                        <label>From:</label>
            ...
            // --- MODIFICATION END ---
                ...
                const optionsMap = {
                    ...
                    'person_role': availableOptions.person_roles,
                    'institution_name': availableOptions.institution_names,
                    'institution_place': availableOptions.institution_places,
                    'event_name': availableOptions.event_names,
                };
                const configMap = {
...
function updateFieldSelector(selectElement) {
    ...
    if (currentEntity === 'work') {
        ...
        selectElement.innerHTML = `
            ...
            <option value="person_gender">Gender</option>
            <option value="person_dates">Dates the person lived</option>
        `;
    } else if (currentEntity === 'visual_object') {
        selectElement.innerHTML = `
            <option value="">-- Select Field --</option>
            <option value="visual_object_transcription">Text search</option>
        `;
    } else if (currentEntity === 'institution') {
        selectElement.innerHTML = `
            <option value="">-- Select Field --</option>
            <option value="institution_name">Name of the institution</option>
            <option value="institution_place">Place of institution</option>
        `;
    } else if (currentEntity === 'event') {
        selectElement.innerHTML = `
            <option value="">-- Select Field --</option>
            <option value="event_name">Name of event</option>
        `;
...
function handleEntityChange() {
    ...
    // Hide everything that is not the main project selector
...
function createEntityLink(id, type, label) {
    ...
    switch (type) {
        ...
        case 'event':
            return `<a href="#" class="event-link" data-event-id="${id}">${label}</a>`;
        case 'expression':
...
async function renderEntityDetailsPage(endpoint, titleKey, entityType, voRoleRules = []) {
    ...
    try {
        ...
        // --- MODIFICATION START: Filter and order details for persons ---
        let otherRelationsHTML = '';
        if (entityType === 'person') {
            const relsByType = otherRels.reduce((acc, rel) => {
                if (!acc[rel.type]) acc[rel.type] = [];
                acc[rel.type].push(rel); // Store the full relationship object
                return acc;
            }, {});
            ...
            let detailsContent = renderOrder.map(item => {
                ...
                let valueStr = '';
                // Special handling for date fields to combine with notes
...
// --- MODIFIED: Functions for Graph Visualization ---

function setupGraphFilters() {
    const container = document.getElementById('graph-filters-container');
    // Prevent re-creating filters if they already exist
    ...
    typeSelectorRow.innerHTML = `
        ...
        <select id="graph-type-select">
            ...
            <option value="person_centric">Person Centric</option>
        </select>
    `;
    container.appendChild(typeSelectorRow);

    // --- 2. Containers for different filter sets ---
    ...
    mentionsFiltersContainer.style.display = 'none'; // Hidden by default
    ...
    personCentricFiltersContainer.style.display = 'none'; // Hidden by default
    container.appendChild(personCentricFiltersContainer);


    // --- 3. Populate General Filters ---
    ...
    const relationships = [
        ...
        'manifestation_has_volume', 'work_authored_by_person', 'work_authored_by_institution',
        'item_contains_physical_object', 'visual_object_inscribed_by_person', 'visual_object_sent_by_person',
    ...
    const workTitleSelect = createCustomMultiSelect(availableOptions.work_titles, 'graph_work_titles', {
        placeholder: '-- Select Title of the Work --',
        hideEmpty: true
    });
    workTitleSelect.id = 'graph-work-title-select';
    
    generalRow.appendChild(entitySelect);
    generalRow.appendChild(relationshipSelect);
    generalRow.appendChild(workTitleSelect);
    generalFiltersContainer.appendChild(generalRow);

    // --- 4. Populate Mentions Filters ---
    ...
    const mentionsWorkTitleSelect = createCustomMultiSelect(availableOptions.work_titles, 'mentions_work_titles', {
        placeholder: '-- Select Title of the Work (Optional) --',
        hideEmpty: true
    });
    mentionsWorkTitleSelect.id = 'mentions-work-title-select';

    mentionsRow.appendChild(mentionsEntitySelect);
    mentionsRow.appendChild(mentionDirectionSelect);
    mentionsRow.appendChild(mentionsWorkTitleSelect);
    mentionsFiltersContainer.appendChild(mentionsRow);

    // --- 5. Populate Person Centric Filters ---
    ...
    const personCentricRelationships = [
        'work_authored_by_person', 'manifestation_edited_by_person', 'Mentioning', 
    ...
    const personNameSelect = createCustomMultiSelect(availableOptions.all_people, 'person_centric_names', {
    ...
    // --- 6. Event Listener to toggle filter visibility ---
    document.getElementById('graph-type-select').addEventListener('change', (e) => {
        ...
        fetchAndRenderGraph(); // Re-fetch when type changes
    });
}
...
async function fetchAndRenderGraph() {
    ...
    try {
        ...
        renderGraph(data, graphType); // Pass graphType to the render function
...
// --- MODIFICATION: Updated edge rendering to account for arrows ---
function updateEdge(edgeEl, labelEl, sourcePos, targetPos, nodeRadius) {
    ...
    // Shorten the line so it ends at the node's border, not its center
...
// --- MODIFICATION START: Refactored drag handlers and collision logic ---

// Helper to attach drag handlers to a node
function attachNodeDragHandlers(nodeEl, nodeId) {
    let activeNode = null;
    let dragOffset = { x: 0, y: 0 };
    const nodeRadius = 80 / 2; // from CSS
...
// Helper to run collision detection on a set of nodes
function runCollisionDetection(nodesToCollide) {
    const nodeSize = 80;
    const nodeRadius = nodeSize / 2;

    for (let i = 0; i < 15; i++) { // More iterations for better separation
        for (let j = 0; j < nodesToCollide.length; j++) {
            for (let k = j + 1; k < nodesToCollide.length; k++) {
                ...
                const minDistance = nodeSize + 40; // Keep a good margin
                ...
                if (distance < minDistance) {
                    ...
                    // Update element positions immediately
    ...
    // Final pass to update all edges connected to the moved nodes
...
// --- MODIFIED: Complete rewrite of renderGraph for a much better layout ---
function renderGraph(data, graphType) {
    ...
    // --- 2. Run Force-Directed Layout on Each Component ---
    ...
    // --- 3. Arrange Components and Run Layouts ---
    ...
    edges.forEach(edge => {
        ...
        // --- NEW: Add classes for mention edges ---
    ...
    try {
        ...
        initialRow.innerHTML = `
            ...
            <select id="entity-select">
                ...
                <option value="visual_object">Visual Object</option>
        ...
        // --- MODIFIED: Separated click and dblclick listeners ---
        document.body.addEventListener('click', (e) => {
            ...
            const eventLink = e.target.closest('.event-link, [data-event-id]'); // ADDED
        ...
        // --- MODIFICATION START: Added visual_object, event, and abstract_character to the dblclick handler ---
